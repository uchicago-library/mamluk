...
<ul>
  ...
  <li><font size="-1">For explanations and examples of search features, please see the <a href="searchhelp.html">expanded search help</a> page.</font></li>
</ul>
...
function item_display($line)
{
    ...
    switch ($str_form) {
case "Book long":
    $str_display = trim($line_comma[3]) . ' ' . trim($line_dot[4]) . ' "' . trim($line_dot[5]) . '" In <I>' . trim($line[8]) . '</I>. ' . trim($line_dot[13]) . ' ' . trim($str_authrole) . ' ' . trim($line_dot[6]) . ' ' . trim($line[16]) . trim($line_dot[15]) . trim($line_dot[22]) . ' ' . trim($line[17]) . ': ' . trim($line_comma[18]) . ' ' . trim($line_dot[19]);
    break;
case "Book short":
...
$query_count = $query_count_pre . $query . " and (STATUS='no')";
$query = $query_pre . $query . " and (STATUS='no') ORDER BY ID LIMIT " . $limit . " OFFSET " . ($start - 1);
...
//while (($line = mysql_fetch_array($result, MYSQL_ASSOC)) && ($i <= $limit)) {
